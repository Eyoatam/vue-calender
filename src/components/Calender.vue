        ...
        >
          <v-card color="grey lighten-4" min-width="350px" flat>
            <v-toolbar :color="selectedEvent.color" dark>
              <v-btn @click="deleteEvt(selectedEvent.id)" icon>
...
export default {
  ...
  methods: {
    async getEvents() {
      let snapshot = await db.collection("calenderEvt").get();
      let events = [];
    ...
    async addEvent() {
      if (this.name && this.start && this.end) {
        await db.collection("calenderEvt").add({
          name: this.name,
        ...
        alert("Name,Start and end are requireq ");
      }
    },
    ...
    editEvent(evt) {
      this, (this.currentlyEditing = evt.id);
    },
